...
fn part_two(rules: &[Rule], ticket: &Ticket, nearby: &[Ticket]) -> u64 {
    ...
    let tickets: Vec<_> = nearby.iter()
        .filter(|t| valid_ticket(t, rules)).collect();

    let mut fields = HashMap::new();
    let mut unused: HashSet<_> = (0..ticket.fields.len()).collect();
    let mut queue: VecDeque<_> = rules.iter().collect();

    while let Some(rule) = queue.pop_front() {
        let valid: Vec<_> = unused.iter().filter(|&n|
            tickets.iter().all(|&t| valid_field(rule, t.fields[*n]))
        ).map(|n| *n).collect();
    ...
    fields.iter().filter(|(r, _)| r.starts_with("departure"))
        .map(|(_, &i)| ticket.fields[i] as u64).product()
}
...
fn load(input: &str) -> (Vec<Rule>, Ticket, Vec<Ticket>) {
    ...
    let tickets: Vec<_> = iter.next().unwrap().lines().skip(1)
...
fn parse_rule(rule: &str) -> Rule {
    ...
    let valid = parts[1].split(" or ").map(parse_range).collect();

    Rule { name: parts[0], valid }
}

fn parse_range(range: &str) -> (u32, u32) {
    let vals: Vec<_> = range.split('-')
